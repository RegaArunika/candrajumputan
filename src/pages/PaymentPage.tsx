...
import type { BookingFormData, CartItem, Jumputan } from "../Types/type";
...
export default function PaymentPage() {
  ...
  const fetchJumputanDetails = async (cartItems: CartItem[]) => {
    try {
      ...
      setJumputanDetails(fetchedDetails);
      setLoading(false);

      const jumputanIdsWithQuantities = cartItems.map((cartItem) => ({
        id: cartItem.jumputan_id,
        quantity: cartItem.quantity,
      }));

      setFormData((prevData) => ({
        ...prevData,
        jumputan_ids: jumputanIdsWithQuantities,
      }));
    } catch (error) {
      console.error("Error fetching jumputan details");
      setError("Failed to fetch jumputan details");
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    setCart(cartItems);
    fetchJumputanDetails(cartItems);
  }, [navigate]);
  ...
  const handleSumbit = async (e: React.FormEvent) => {
    ...
    formData.jumputan_ids.forEach((item, index) => {
      submissionData.append(`jumputan_ids[${index}][id]`, String(item.id));
      submissionData.append(
        `jumputan_ids[${index}][quantity]`,
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        console.log("Transaction respone data:", response.data.data);
        ...
        setFormData({ proof: null, jumputan_ids: [] });
        setLoading(false);
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-[640px] flex-col gap-5 bg-[#F6F6F8] pb-[30px]">
      ...
      <form onSubmit={handleSumbit} className="flex flex-col gap-5 px-5">
